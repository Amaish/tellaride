...
<head>
	<meta charset="utf-8">
	<title><?php echo $adminName; ?> - Contacts</title>
         ...
         while (($emapData = fgetcsv($file, 100000, ",")) !== FALSE)
         {
            if (!empty($emapData[0]) AND !empty($emapData[1]) AND !empty($emapData[2]) AND !empty($emapData[3]))
			{
                if(strpos($emapData[1], '+') !== false) 
                {
                    ...
                    $arguments = array('phonenumber' => $emapData[1]);
                    ...
                    echo '<a class="list-group-item text-danger" href="#">
                                        <span class="pull-right text-danger">
                                            <i class="ion-close-circled text-xs"></i>
                                        </span> The phone number <b>'.$emapData[0].', '.$emapData[1].'</b> should be in international format
                    </a>';
                }
            }else{
                echo '<a class="list-group-item text-danger" href="#">
